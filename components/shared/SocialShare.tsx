  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {platforms.map((p) => {
          ...
          if (isInsta) {
            return (
              <button
                ...
                className="flex flex-col items-center gap-1.5 p-3 rounded-xl text-white transition-all duration-200 relative overflow-hidden"
                style={{
              ...
              >
                {instaCopied ? <Check size={22} /> : p.icon}
                <span className="text-[10px] font-semibold leading-tight">
                  {instaCopied ? 'Copied!' : p.label}
                </span>
              </button>
            )
          }
          ...
          return (
            <a
              ...
              onMouseEnter={() => setHovered(p.id)}
              onMouseLeave={() => setHovered(null)}
              className="flex flex-col items-center gap-1.5 p-3 rounded-xl text-white transition-all duration-200 no-underline"
              style={{
                background: isHovered ? (p.hover as string) : (p.bg as string),
                transform: isHovered ? 'translateY(-2px)' : 'translateY(0)',
                boxShadow: isHovered ? '0 6px 16px rgba(0,0,0,0.2)' : '0 2px 6px rgba(0,0,0,0.1)',
        ...
        >
          {copied ? <Check size={22} style={{ color: '#059669' }} /> : <Link2 size={22} />}
          <span className="text-[10px] font-semibold leading-tight">
      ...
      {/* Instagram hint */}
      {instaCopied && (
        <p className="text-xs text-center animate-pulse" style={{ color: 'var(--ep-muted)' }}>
